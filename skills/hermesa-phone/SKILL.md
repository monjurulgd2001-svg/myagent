...
- **Database URL**: auto-configured - `HERMESA_DB_URL` in `~/.hermes/.env`
  (default `https://softizence-default-rtdb.firebaseio.com`)
- **Webhook endpoint** (per bot):
  `POST https://softizence-default-rtdb.firebaseio.com/bots/<BOT_ID>/messages.json`
...
```bash
curl -X POST "https://softizence-default-rtdb.firebaseio.com/bots/$BOT_ID/messages.json" \
