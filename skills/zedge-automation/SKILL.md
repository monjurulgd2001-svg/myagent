...
## Commands (`zedge` terminal command)
    ...
    zedge vpn up|down|status -i N    # browser-only OpenVPN (netns)
...
8. VPN - BROWSER-ONLY (.ovpn): paste each account's OpenVPN config
   into ZEDGE_OVPN_1/2/3 (+ "user : pass" on line N of
   ZEDGE_OVPN_AUTH if the VPN needs credentials). `zedge run` then
   ...
   no IP leak). When both a .ovpn and a proxy line exist, the VPN
